...
class KillClipperApp:
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header, text="⚔ 王者剪辑", font=("Segoe UI", 28, "bold"), text_color=ACCENT).pack()
        ctk.CTkLabel(header, text="基于 EasyOCR 的全自动击杀追踪框架", font=FONT_SM, text_color="gray").pack()

        # 拖拽区
        self._build_drop_zone(main_frame)

        # 参数设置
        self._build_params(main_frame)

        # ROI 区域
        self._build_roi(main_frame)

        # 调试勾选框
        ctk.CTkSwitch(
            main_frame, text="开启调试模式（实时显示 AI 视觉窗口，排查误报）",
        ...
        self.btn_run = ctk.CTkButton(
            main_frame, text="▶  开始批量剪辑",
            font=("Segoe UI", 16, "bold"),
            fg_color=ACCENT, text_color="black", hover_color="#dddddd",
            height=46, corner_radius=8,
            command=self._on_run
        )
        self.btn_run.pack(fill="x", pady=(10, 5))

        # 进度条与终止按钮
        ctrl_row = ctk.CTkFrame(main_frame, fg_color="transparent")
        ctrl_row.pack(fill="x", pady=(0, 15))
        
        self.progress_bar = ctk.CTkProgressBar(ctrl_row, progress_color=ACCENT, height=10)
        self.progress_bar.pack(side="left", fill="x", expand=True, padx=(0, 10))
        self.progress_bar.set(0)

        self.btn_stop = ctk.CTkButton(
            ctrl_row, text="终止", width=80, height=30,
